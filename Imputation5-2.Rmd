...
Based on what we get in "Imputation5-new-.Rmd" file, we let pregnow_r==1, currmarr_r==1 and educate_r==0 be student_r == 0, and use AUC instead of missclassification.  
...
Let pregnow_r==1, currmarr_r==1 and educate_r==0 be student_r == 0.

```{r}
student_data = new_data %>%
  as.data.frame() %>% 
  mutate(student_r = ifelse(occup1_r == 8 | occup2_r == 8, 1, 0),
         student_r = ifelse(is.na(student_r) & occup1_r == 20, 0, student_r))
#head(student_data)
added_data = student_data %>% 
  mutate(student_r = ifelse(is.na(student_r)&pregnow_r==1, 0, student_r),
         student_r = ifelse(is.na(student_r)&currmarr_r==1, 0, student_r),
         student_r = ifelse(is.na(student_r)&educate_r==0, 0, student_r)) 
add_data = student_data %>% 
  filter(is.na(student_r)) %>% 
  filter(pregnow_r==1|currmarr_r==1|educate_r==0)
```

There are `r nrow(add_data)` student_r observation be marked as 0.
...
```{r}
target_data = added_data %>% 
  filter(ageyrs_r <= 19 & ageyrs_r>=15) %>% 
  mutate(student_r = as.factor(student_r),
         visit = as.factor(visit),
         gender = ifelse(female==1, "female", "male")) %>% 
  select(student_r, visit, ageyrs_r, gender, area, educate_r, currmarr_r, sexp1yr, SEScat, pregnow_r, eversex_r, currrltn)
summary(target_data)
```
...
sexp1yr; <=2 -> 92, >2 -> 93, 97,98,99 -> NA

pregnow_r; NA	For NA with sex equals to male -> 98(not applicable)

currrltn(Do you currently have a sexual relationship with someone to whom  you are not officially married or in a consensual union?)	0,8,9 -> NA

```{r}
impu_data = target_data %>% 
  mutate(sexp1yr = ifelse(sexp1yr %in% c(97, 98, 99), NA, sexp1yr),
         sexp1yr = ifelse(sexp1yr<=2, "a few(1-2)", "a lot(3+)"),
         sexp1yr = as.factor(sexp1yr)) %>% 
...
mean(res_error$himsc)
mean(res_error$forest)
mean(res_error$mice)
```
